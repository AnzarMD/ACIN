...
const ROUTE_LABELS: Record<string, { label: string; color: string }> = {
  ...
  WAREHOUSE:         { label: "Central Warehouse", color: "text-gray-600" },
...
export default function LogisticsCard({ analysis }: { analysis: LogisticsAnalysis }) {
  const route = ROUTE_LABELS[analysis.selected_route] || { label: analysis.selected_route, color: "text-gray-600" };
  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm space-y-5">
      ...
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-semibold text-gray-900">Logistics Routing</h3>
      ...
      <div className="bg-gray-50 rounded-lg p-4">
        <p className="text-xs text-gray-500 uppercase font-semibold mb-1">Selected Route</p>
      ...
      {analysis.alternatives && analysis.alternatives.length > 0 && (
        <div>
          <p className="text-xs text-gray-500 font-semibold uppercase mb-1">Alternatives Considered</p>
          <div className="flex flex-wrap gap-2">
            {analysis.alternatives.map((alt) => (
              <span key={alt} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">
      ...
      {analysis.rule_version && (
        <p className="text-xs text-gray-400">{analysis.rule_version}</p>
      )}
    </div>
  );
}
